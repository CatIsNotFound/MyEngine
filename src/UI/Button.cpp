...
namespace MyEngine {
    namespace UI {
        ...
        void Button::onLeft() {
            ...
            _rect.setBackgroundColor(
                    _checkable && _is_checked ? _btn_skin.background.checked : _btn_skin.background.normal);
            if (_is_valid)
                TextSystem::global()->setTextColor(_id, _checkable && _is_checked ?
        ...
        void Button::resizeEvent() {
            ...
            const auto* r = _click_area.getRect();
            _rect.resize(r->geometry().size);
        }

        void Button::moveEvent() {
            AbstractControl::moveEvent();
            const auto* r = _click_area.getRect();
            _rect.move(r->geometry().pos);
        }
        ...
        void Button::onChecked(bool checked) {
            ...
            if (checked) {
                ...
                if (_is_valid)
                    TextSystem::global()->setTextColor(_id, _click_area.isHovered() ?
