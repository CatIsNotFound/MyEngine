...
namespace MyEngine {
    namespace UI {
        ClickArea::ClickArea(uint64_t window_id, GT graphic)
            ...
            EventSystem::global()->appendEvent(IDGenerator::getNewEventID(), [this](SDL_Event ev) {
        ...
        void ClickArea::setViewportArea(int x, int y, int w, int h) {
            ...
            if (std::holds_alternative<Graphics::Point>(_real_base)) {
                ...
                Logger::log("ClickArea::setViewportArea: Failed to set viewport area! The graphic is not valid!",
                            Logger::Warn);
            }
        }
        ...
        const Graphics::Point* ClickArea::getPoint() {
            if (_base.index() != 1) return nullptr;
            return &std::get<Graphics::Point>(_base);
        }
        ...
        const Graphics::Rectangle* ClickArea::getRect() {
            if (_base.index() != 2) return nullptr;
            return &std::get<Graphics::Rectangle>(_base);
        }
        ...
        AbstractControl::AbstractControl(const std::string &name, Renderer *renderer,
                ...
                : _name(name), _renderer(renderer), _parent(parent), _id(IDGenerator::getID(2)),
